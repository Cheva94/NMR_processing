...
def plot(tau1, tau2, Z, T1, T2, S, M1, M2, Out, nLevel, T1min, T1max, T2min, T2max, RGnorm, alpha, Back, m, niniT1, niniT2):
    ...
    for i in range(len(peaks2x)):
        if peaks2y[i] > 0.1 * topPeak2:
            ...
            axs[0,2].annotate(f'{peaks2x[i]:.0f}', xy = (peaks2x[i], peaks2y[i] + 0.1), fontsize=30, ha = 'center')
    ...
    for i in range(len(peaks1x)):
        if peaks1y[i] > 0.1 * topPeak1:
            axs[1,3].plot(peaks1y[i] + 0.03, peaks1x[i], lw = 0, marker=4, color='black')
            axs[1,3].annotate(f'{peaks1x[i]:.0f}', xy = (peaks1y[i] + 0.1, peaks1x[i]), fontsize=30, va = 'center')
    ...
    for x in range(len(cumT1)):
        if cumT1[x] < 0.01:
            ...
            ax.annotate(f'{100*cumT1[x]:.0f} %', xy = (cumT1[x], T1[15:-15][-1]), fontsize=30, ha='right', color='coral')
        ref = cumT1[x]
    ax.set_xlim(-0.1, 1.1)
