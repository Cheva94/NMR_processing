...
def fit_1(t, decay):
    ...
    chi2, pval = chisquare(decay, f_exp = exp_1(t, *popt))

    M0, T2 = popt[0], popt[1]
    M0_SD, T2_SD = perr[0], perr[1]

    return popt, r2, chi2, pval, M0, T2, M0_SD, T2_SD
...
def plot_1(t, decay, popt, echo_time, input_file):
    ...
    ax.text(0.02,0.02, fr'$T_E$={echo_time} ms', ha='left', va='bottom', transform=ax.transAxes, size='small') #medium
...
def out_1(t, decay, echo_time, input_file):
    ...
    popt, r2, chi2, pval, M0, T2, M0_SD, T2_SD = fit_1(t, decay)

    plot_1(t, decay, popt, echo_time, input_file)

    output_file = input_file.split('.txt')[0]
    with open(f'{output_file}_fit-exp1.csv', 'w') as f:
        f.write("M0, M0-SD, T2 [ms], T2-SD [ms], R2, Chi2, pVal \n")
        f.write(f'{M0:.4f}, {M0_SD:.4f}, {T2:.4f}, {T2_SD:.4f}, {r2:.4f}, {chi2:.4f}, {pval:.4f}')
...
def fit_2(t, decay):
    ...
    chi2, pval = chisquare(decay, f_exp = exp_2(t, *popt))

    M0_1, T2_1, M0_2, T2_2 = popt[0], popt[1], popt[2], popt[3]
    M0_1_SD, T2_1_SD, M0_2_SD, T2_2_SD = perr[0], perr[1], perr[2], perr[3]

    return popt, r2, chi2, pval, M0_1, T2_1, M0_2, T2_2, M0_1_SD, T2_1_SD, M0_2_SD, T2_2_SD
...
def out_2(t, decay, echo_time, input_file):
    ...
    popt, r2, chi2, pval, M0_1, T2_1, M0_2, T2_2, M0_1_SD, T2_1_SD, M0_2_SD, T2_2_SD = fit_2(t, decay)

    plot_2(t, decay, popt, echo_time, input_file)

    output_file = input_file.split('.txt')[0]
    with open(f'{output_file}_fit-exp2.csv', 'w') as f:
        f.write("M0_1, M0_1-SD, T2_1 [ms], T2_1-SD [ms], M0_2, M0_2-SD, T2_2 [ms], T2_2-SD [ms], R2, Chi2, pVal \n")
        f.write(f'{M0_1:.4f}, {M0_1_SD:.4f}, {T2_1:.4f}, {T2_1_SD:.4f}, {M0_2:.4f}, {M0_2_SD:.4f}, {T2_2:.4f}, {T2_2_SD:.4f}, {r2:.4f}, {chi2:.4f}, {pval:.4f}')
...
def fit_3(t, decay):
    ...
    chi2, pval = chisquare(decay, f_exp = exp_3(t, *popt))
    ...
    return popt, r2, chi2, pval, M0_1, T2_1, M0_2, T2_2, M0_3, T2_3, M0_1_SD, T2_1_SD, M0_2_SD, T2_2_SD, M0_3_SD, T2_3_SD
...
def out_3(t, decay, echo_time, input_file):
    ...
    popt, r2, chi2, pval, M0_1, T2_1, M0_2, T2_2, M0_3, T2_3, M0_1_SD, T2_1_SD, M0_2_SD, T2_2_SD, M0_3_SD, T2_3_SD = fit_3(t, decay)

    plot_3(t, decay, popt, echo_time, input_file)

    output_file = input_file.split('.txt')[0]
    with open(f'{output_file}_fit-exp3.csv', 'w') as f:
        f.write("M0_1, M0_1-SD, T2_1 [ms], T2_1-SD [ms], M0_2, M0_2-SD, T2_2 [ms], T2_2-SD [ms], M0_3, M0_3-SD, T2_3 [ms], T2_3-SD [ms], R2, Chi2, pVal \n")
        f.write(f'{M0_1:.4f}, {M0_1_SD:.4f}, {T2_1:.4f}, {T2_1_SD:.4f}, {M0_2:.4f}, {M0_2_SD:.4f}, {T2_2:.4f}, {T2_2_SD:.4f}, {M0_3:.4f}, {M0_3_SD:.4f}, {T2_3:.4f}, {T2_3_SD:.4f}, {r2:.4f}, {chi2:.4f}, {pval:.4f}')
...
def out_multi(t, decay, echo_time, input_file):
    '''
    Fits mono-, bi- and tri- exponential decay to choose best fit. Also save optimized parameters with statistics and plot.
    '''

    popt_mono, r2_mono, chi2_mono, pval_mono, M0_mono, T2_mono, M0_SD_mono, T2_SD_mono = fit_1(t, decay)

    popt_bi, r2_bi, chi2_bi, pval_bi, M0_1_bi, T2_1_bi, M0_2_bi, T2_2_bi, M0_1_SD_bi, T2_1_SD_bi, M0_2_SD_bi, T2_2_SD_bi = fit_2(t, decay)

    popt_tri, r2_tri, chi2_tri, pval_tri, M0_1_tri, T2_1_tri, M0_2_tri, T2_2_tri, M0_3_tri, T2_3_tri, M0_1_SD_tri, T2_1_SD_tri, M0_2_SD_tri, T2_2_SD_tri, M0_3_SD_tri, T2_3_SD_tri = fit_3(t, decay)
    ...
    with open(f'{output_file}_fit-all.csv', 'w') as f:
        f.write("Components, R2, Chi2, pVal, M0_1, M0_1-SD, T2_1 [ms], T2_1-SD [ms], M0_2, M0_2-SD, T2_2 [ms], T2_2-SD [ms], M0_3, M0_3-SD, T2_3 [ms], T2_3-SD [ms] \n")
        f.write(f'Mono, {r2_mono:.4f}, {chi2_mono:.4f}, {pval_mono:.4f}, {M0_mono:.4f}, {M0_SD_mono:.4f}, {T2_mono:.4f}, {T2_SD_mono:.4f} \n')
        f.write(f'Bi, {r2_bi:.4f}, {chi2_bi:.4f}, {pval_bi:.4f}, {M0_1_bi:.4f}, {M0_1_SD_bi:.4f}, {T2_1_bi:.4f}, {T2_1_SD_bi:.4f}, {M0_2_bi:.4f}, {M0_2_SD_bi:.4f}, {T2_2_bi:.4f}, {T2_2_SD_bi:.4f} \n')
        f.write(f'Tri, {r2_tri:.4f}, {chi2_tri:.4f}, {pval_tri:.4f}, {M0_1_tri:.4f}, {M0_1_SD_tri:.4f}, {T2_1_tri:.4f}, {T2_1_SD_tri:.4f}, {M0_2_tri:.4f}, {M0_2_SD_tri:.4f}, {T2_2_tri:.4f}, {T2_2_SD_tri:.4f}, {M0_3_tri:.4f}, {M0_3_SD_tri:.4f}, {T2_3_tri:.4f}, {T2_3_SD_tri:.4f}')
