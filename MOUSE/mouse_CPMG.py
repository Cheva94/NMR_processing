...
def main():
    ...
    print('Processing...')
    if Back == None:
        S0, T2, tau, K, decay, nP, shEcho, tEcho, nEcho, topEcho, nS, pulse, RD = CPMG_file(File, T2min, T2max, nini)
        Z = PhCorr(decay)
    ...
    print('Plotting...')
    plot(tau, Z, MLaplace, T2, S, Out, shEcho, tEcho, nEcho, topEcho, nS, pulse, RD, alpha, Back, cumT2, nini, T2min, T2max, dataFit)
...
#    parser.add_argument('output', help = "Path for the output files.")
    ...
    parser.add_argument('-crop', '--croppedValues', help = "Number of values to avoid at the beginning of T2.", type = int, default=0)
