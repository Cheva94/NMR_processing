...
def CPMG_file(File, T2min, T2max, nini):
    ...
    shEcho = float(pAcq.iloc[10, 1])
    tEcho = float(pAcq.iloc[11, 1])
    nEcho = float(pAcq.iloc[20, 1])
    topEcho = float(pAcq.iloc[23, 1])
    nS = float(pAcq.iloc[24, 1])
    pulse = float(pAcq.iloc[27, 1])
    RD = float(pAcq.iloc[29, 1])

    return S0, T2, tau[nini:], K, decay[nini:], nP, shEcho, tEcho, nEcho, topEcho, nS, pulse, RD
...
def plot(tau, Z, MLaplace, T2, S, Out, shEcho, tEcho, nEcho, topEcho, nS, pulse, RD, alpha, Back, cumT2, nini, T2min, T2max, dataFit):
    '''
    Grafica resultados.
    '''
    ...
    axs[0,2].fill_between([tEcho/1000, 5 * tEcho/1000], -0.02, 1.2, color='red', alpha=0.3, zorder=-2)
